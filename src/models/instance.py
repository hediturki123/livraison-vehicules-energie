...
class Instance:
    ...
    def execute(self) -> None:
        visits_to_do: list[int] = [visit_id for visit_id in self.visits]
        visits_to_do.remove(WAREHOUSE_POSITION)
        # On crée un nouveau véhicule
        current_vehicle: Vehicle = Vehicle(self.distances, self.times)
        ...
        while len(visits_to_do) > 0:
            ...
            if current_vehicle.can_move_and_deliver(next_visit, self.visits[next_visit].demand):
                ...
                if current_vehicle.can_move_and_deliver(next_visit, self.visits[next_visit].demand):
                    ...
                    current_vehicle = Vehicle(self.distances, self.times)
